...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Update status
    $update_query = "UPDATE pembelian SET status = ? WHERE id = ?";
    $update_stmt = $conn->prepare($update_query);
    $update_stmt->bind_param("si", $status, $id); // 's' untuk string, 'i' untuk integer (id)

    if ($update_stmt->execute()) {
        echo "<script>alert('Data berhasil diperbarui!'); window.location.href='dashboard_data_pembelian_obat.php';</script>";
    } else {
